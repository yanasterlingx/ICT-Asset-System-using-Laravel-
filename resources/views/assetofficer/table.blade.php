<div class="row">
              ...
              <ul class="nav nav-pills flex-column">
                      @foreach ($depart as $cat)
                        <li class="nav-item {{ $cat->id == $id ? 'active' : ''}} ">
                          <a class="nav-link" href="/assetofficer/department/{{ $cat->id }}">{{ $cat->deptname }}</a>
              ...
              <ul class="nav nav-pills flex-column">
                      @foreach ($devices as $c)
                        <li class="nav-item {{ $c->id == $id ? 'active' : ''}} ">
                          <a class="nav-link" href="/assetofficer/device/{{ $c->id }}">{{ $c->device_name }}</a>
                        </li>
                      @endforeach
                    </ul>
              </div>
    ...
    @endif
              ...
              <div class="card col-md-12">
                <div class="card-header">
                <a class="btn btn-success" href="{{ route('assetofficer.create') }}"> Tambah Inventori Asset</a>
                ...
                <table id="example2" class="table table-bordered table-hover">
                  ...
                  <tbody>
                    @if($assets->count())
                      ...
                      <td>{{ $t->updated_at}}</td>
                      <td>
                      <form action="{{ route('assetofficer.destroy',$t->id) }}" method="POST">

                      <a class="btn btn-primary" href="{{ route('assetofficer.edit',$t->id) }}"><i class="fa fa-edit"></i></a>
